...
# We want to list all Filesets that have multiple images
# (not including '[macro]' '[label]' images)
with open('fileset_images.csv', mode='w') as csv_file:
    ...
    for d in datasets:
        ...
        images = requests.get(img_url, timeout=10).json()['images']
        filesets = {}
        ...
        for fs_imgs in filesets.values():
            imgs = [i for i in fs_imgs if "[macro" not in i['name'] and "[label" not in i['name']]
            if len(imgs) > 1:
                mi_filesets += 1
                for i in imgs:
                    csv_writer.writerow([i['name']])
        ...
        print('multi-image_filesets', mi_filesets)
